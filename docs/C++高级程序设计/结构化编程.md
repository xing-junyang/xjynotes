...
在类型转换时，经常会出现**溢出**和**精度损失**的问题。一般来说，对于相同的数据存储模式，当我们把大的类型转换为小的类型时，就通常会出现溢出的问题，而当我们把小的类型转换为大的类型时，则一般没有问题。但我们如果把 `float` 类型转换为 `int` 类型时，可能出现溢出或精度损失的问题；而把 `int` 类型转换为 `float` 类型时，可能出现精度损失的问题。自然，将 `double` 类型转换为 `int` 类型时也会有相关问题。一般来说，我们应当尽量避免这些问题，尤其是要及时识别可能的隐式转换。
...
- `auto` 关键字：可以使用 `auto` 关键字来避免冗余的类型定义。**但一定要时刻检测推导出的类型，防止出现隐式转换**。
...
指针也可以指向一个多维数组。例如 `int (*p)[2][3]` 就指向了 `int [2][3]` 这一个 $2\times 3$ 的二维 `int` 数组。

如果理解了上述规则，可以尝试思考，`char (*(*a[10]))[3]` 代表的是什么？

```
请思考后，将鼠标移动到下一行查看答案。// [!code focus] // [!code ++]
一个指向长度为 10 的指针数组的指针，其中，这一指针数组里面的每个元素都指向一个长度为 3 的 char 数组。
```
...
函数是一小段代码的集合，它可以被调用，也可以被传递。函数在使用前必须被定义或声明；若声明，必须存在函数定义。此外，与其他语言不同，函数的定义**不允许嵌套**。

::: tip 声明和定义的区别

对于函数：
- 声明指的是向编译器表明某个函数或变量的存在，但不具体描述其实现细节，也称为函数的原型。
- 定义则包含了函数的实现，即具体代码。
...
头文件通常包括**常量定义 `const`**、**类型定义 `typedef`**、**预处理指令**、**（内联）函数声明**等。一般来说，头文件中不包含函数的具体实现，而只包含函数的声明。
