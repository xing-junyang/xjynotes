...
了解 `location` 的匹配顺序对于配置优化非常重要，因为一旦找到匹配的 `location` 块，Nginx 将停止进一步的匹配。
:::
...
缓存这一部分与计算机中的 `Cache` 有很大的相似性，许多概念，包括**缓存命中率、替换算法**等都是相通的。缓存还有两种使用模式 `Cache Aside` 和 `Cache as SoR`，前者指业务代码**主动维护缓存**，后者指缓存作为数据源，真实的数据源不可见，只能**通过缓存访问**。

此外，还可以通过**分布式缓存解决热点数据问题**。
