...
**Scrum** 是一种迭代式增量软件开发过程，通常用于敏捷软件开发。它既可以应用于**管理软件开发项目**，也可以用于**运行软件维护团队**。传统软件开发迭代速度慢，相当于寻求的是**全局最优解**；而敏捷开发则是寻求**局部最优解**，通过迭代的方式逐步优化。从企业的商业目标来看，并考虑到市场等因素的不断变化，敏捷开发的优势就显而易见了。
...
**精益思想**：从以**资源效率**到以**流动效率**为核心的演进。资源效率一般指各个资源的使用率，其关注点在局部的资源和职能；而流动效率指各个流动单元之间价值流动的速度，其关注点在于整个系统端到端的价值。