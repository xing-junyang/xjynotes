...
- 基础设施即服务（$\mathrm{IaaS}$）：如云虚拟化主机等。其中系统与应用由自己管理。
- 平台即服务（$\mathrm{PaaS}$）：制造商提供一个开发环境供应用开发者开发，应用及数据由自己管理。如微信小程序平台等。
- 部署频率：组织部署新代码的频率。**部署频率越高，价值流动越快，$\mathrm{DevOps}$ 能力越强。**
- Lead time for changes：从代码提交到代码成功运行在产品中的时间。
- 平均恢复时间 $\mathrm{MTTR}$：当服务出现紧急事件时，恢复的平均时间。
...
- **微服务**：一种将软件应用设计成一套独立的可部署的服务的方法。这种设计方法有共同的特征，例如**自动化部署、endpoint intelligence 以及代码和数据的去中心化**控制。
- 金丝雀测试 $\mathrm{A/B~Testing}$：当新的特性或多个变化待发布时，先将它们投放至不同用户集中，然后量化其指标来做出决定。**灰度测试**
...
利用 $\mathrm{DevOps}$ 现代软件工程的方法、工具、实践来指导软件系统开发和服务运维，并逐渐进行标准化建设。

## 服务标准

在 $\mathrm{DevOps}$ 中，$\mathrm{Ops}$ 端最常见的形式就是提供各种服务给用户，那么就需要遵循一定的标准和要求。比较常用的服务有：$\mathrm{CMMI-SVC}$、$\mathrm{ITIL}$、$\mathrm{ISO20000}$ 等。

- $\mathrm{CMMI-SVC}$：$\mathrm{CMMI}$（$\mathrm{Capability~Maturity~Model~Integration}$）模型是为组织改进流程最佳实践的集合。这些模型由来自行业、政府和软件工程研究所的成员组成的产品团队开发。该模型被称为 $\mathrm{CMMI}$ 服务（$\mathrm{CMMI-SVC}$），为优质服务提供了一套全面的指南。
- $\mathrm{ITIL}$：即信息技术基础架构库。其最佳实践围绕五个部分：**服务战略、服务设计、服务转换、服务运营和服务改进**。
- $\mathrm{ISO20000}$：定义了“策划-实施-检查-处置”的方法论用于服务管理体系和服务的所有部分。
- $\mathrm{ITSS}$：信息技术服务标准是一套国产的成体系和综合配套的信息技术服务标准库。
