...
- $\frac{\partial \mathbf{x^TAx}}{\partial \mathbf{x}} = \mathbf{A}\mathbf{x}+\mathbf{A}^T\mathbf{x}$，这是因为 
$$
[\frac{\partial \mathbf{x^TAx}}{\partial \mathbf{x}}]_k = \frac{\partial \sum_{i}\sum_{j}x_ix_jA_{ij}}{\partial x_k}= \sum_{i=1}^nx_i(A_{ik}+A_{ki})
$$
