...
例如，对于抛硬币的例子，已知事件 $A$ 是“抛十次硬币得到五次正面”，事件 $B$ 是“硬币集中在正面的分布是 $b$，即硬币抛出一次后，得到正面的概率是 $b$”。那么，我们可以定义似然函数为
...
- $p(y = i)$ 是类别 $i$ 的先验概率；
- $p(\mathbf{x})$ 是特征 $\mathbf{x}$ 的先验概率；
- $p(\mathbf{x}|y = i)$ 是类别 $i$ 的似然函数；
- $p(y = i|\mathbf{x})$ 是类别 $i$ 的后验概率；


## 距离度量函数

对于两个样本向量 $x_i, x_j \in \mathbb{R}^d$，我们希望计算它们之间的偏离程度。这就是距离度量函数。常见的各个距离度量函数的计算方法如下：
...
> [!NOTE] 范数
> 
> 范数是一个函数，它将向量映射到非负值。对于一个向量 $\mathbf{x} \in \mathbb{R}^d$，它的 $p$ 范数定义为：
> 
> $$
> ||\mathbf{x}||_p = \left ( \sum_{i=1}^d|x_i|^p \right )^{1/p}
> $$
> 
> 上文的曼哈顿距离、欧式距离和切比雪夫距离分别是 $1$ 范数、$2$ 范数和 $\infty$ 范数的特例。

::: details 马氏距离的计算
马氏距离的 $M$ 是**各维度的协方差矩阵**的逆，它是一个 $d\times d$ 的矩阵，其中 $d$ 是向量的维数。

例如，对于三个样本向量 $\mathbf{x}_1, \mathbf{x}_2, \mathbf{x}_3 \in \mathbb{R}^2，\mathbf{x}_1 = [1, 2]^T, \mathbf{x}_2 = [2, 3]^T, \mathbf{x}_3 = [3, 4]^T$，我们可以按照如下方法计算它们之间的马氏距离：

- 计算协方差矩阵 $\Sigma$：
  - 确定各维度向量 $\mathbf{m} = [1, 2, 3]^T$，$\mathbf{n} = [2, 3, 4]^T$；
  - 计算协方差矩阵 $\Sigma = \mathbf{cov}(\mathbf{m}, \mathbf{n})$；
- 计算协方差矩阵的逆 $M = \Sigma^{-1}$；
- 计算马氏距离 $d_M(\mathbf{x}_i, \mathbf{x}_j) = \sqrt{(\mathbf{x}_i-\mathbf{x}_j)^TM(\mathbf{x}_i-\mathbf{x}_j)}$。
...
**高斯分布**是一种连续概率分布，也称为**正态分布**。正态分布是在统计中以及众多统计测试中最广泛应用的一类分布。它的概率密度函数为：

$$
f(x|\mu, \sigma^2) = \frac{1}{\sqrt{2\pi}\sigma}e^{-\frac{(x-\mu)^2}{2\sigma^2}}
$$

其中，$\mu$ 是**均值**，$\sigma^2$ 是**方差**。正态分布的均值和方差分别决定了分布的位置和形状。正态分布的概率密度函数是一个钟形曲线，它的均值是**对称轴**，方差决定了曲线的“胖瘦”。

对于 $d$ 维随机变量 $\mathbf{x} \in \mathbb{R}^d$ 服从多元高斯分布，其概率密度函数为：
...
其中，$\mathbf{\mu}$ 是**均值向量**，$\mathbf{\Sigma}$ 是**协方差矩阵**。
