...
对数几率回归的**损失函数**是**似然函数**的负对数（加一个负号转化为最小化问题），对于数据集 $\{(x_i,y_i)\}_{i=1}^{m}$ 即：
...
其中，$\mathbf{w}, b$ 相当于是参数，而 $p(y_i|\mathbf{x}_i;~\mathbf{w}, b)$ 是对数几率回归预测准确的概率，也就是说，要令每个样本的预测值尽可能接近真实值。关于似然，可以参考[另一篇文章](/机器学习/数学基础补充#条件概率与贝叶斯公式)。
...
线性判别分析 $\text{(Linear Discriminant Analysis, LDA)}$ 将样例投影到一条直线（或低维空间）上，使得同类样例的投影点尽可能接近，异类样例的投影点尽可能远离。也被称为一种**监督降维**技术。
