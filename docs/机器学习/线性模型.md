...
其中，$\mathrm{Sigmoid}$ 函数是一个常用的联系函数，它的**导数形式简单，且具有良好的性质**。

### 对数几率回归（对率回归）

对数几率回归是一种**广义线性回归**，它的联系函数是 $\mathrm{Sigmoid}$ 函数。对数几率回归的模型可以表示为：
...
$$
\ln{\frac{f(\mathbf{x})}{1 - f(\mathbf{x})}} = \mathbf{w}^T\mathbf{x} + b
$$

将 $\ln{\frac{f(\mathbf{x})}{1 - f(\mathbf{x})}}$ 称为**对数几率**，它表示了正类的概率与负类的概率之比，即：
...
\begin{aligned}
\mathcal{L}(\mathbf{w}, b) &= -\ln{\prod_{i=1}^{m} p(y^{(i)}|\mathbf{x}^{(i)};~\mathbf{w}, b)} \\
&= -\sum_{i=1}^{m} \ln{p(y^{(i)}|\mathbf{x}^{(i)};~\mathbf{w}, b)} 
\end{aligned}
...
### 线性判别分析（$\mathrm{Linear~Discriminant~Analysis,~LDA}$）

将样例投影到一条直线（或低维空间）上，使得同类样例的投影点尽可能接近，异类样例的投影点尽可能远离。也被称为一种**监督降维**技术。

例如，给定数据集 $\{(x_i,y_i)\}_{i=1}^{m}$，设第 $i$ 类样本的集合为 $X_i$，第 $i$ 类样本的均值为 $\mu_i$，第 $i$ 类样本的协方差矩阵为 $\Sigma_i$，那么两类样本的中心在直线上的投影点为 $\mathbf{w}^T\mu_0$ 和 $\mathbf{w}^T\mu_1$，两类样本的协方差为 $\mathbf{w}^T\Sigma_0\mathbf{w}$ 和 $\mathbf{w}^T\Sigma_1\mathbf{w}$，那么我们的目标是以下两点：
...
其中，$S_b = (\mu_0 - \mu_1)(\mu_0 - \mu_1)^T$ 是**类间散度矩阵**，$S_w = \Sigma_0 + \Sigma_1$ 是**类内散度矩阵**。
