...
对于这一公式，是不是和高中的**点到直线的距离公式**很像呢？例如，给定直线 $Ax+By+C=0$，点 $(x_0, y_0)$ 到直线的距离为 
...
假设某个最优解为 $(\mathbf w^*, b^*)$，那么我们令一个常数 $c = \min_{1\le i\le n} y_i(({\mathbf w}^{*})^T\mathbf x + b^*)$，显然，$\frac{1}{c}(\mathbf w^*, b^*)$ 也是一个最优解。我们代入后发现

$$
\min_{1\le i\le n} y_i(({\frac{1}{c}\mathbf w}^{*})^T\mathbf x + \frac{b^*}{c}) = 1>0
$$

因此，我们可以限制 $\min_i y_i(\mathbf w^T \mathbf x + b) = 1$，这样我们就可以得到一个更简单的优化问题，并且不改变最优解。如下方示意图，这一限制在图中表现**为两条虚线**（高维空间则为超平面）：

<div style="display: flex; align-items: center; justify-content: center; flex-direction: column; padding-top:10px">
    <img src='/image/Screen Shot 2025-01-06 at 10.29.21 AM.png' alt="" style="width:50%;"></img>
    <p style="font-size: 12px; color: gray;">支持向量与间隔的示意图</p>
</div>

下面问题就变为在限制 $\min_i y_i(\mathbf w^T \mathbf x + b) = 1$ 的情况下，最大化**间隔**，即最大化 $\frac{1}{||\mathbf w||}$。形式化表述又可转为
